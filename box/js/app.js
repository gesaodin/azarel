...
function OpenModalAnimals(){
  ...
  lblNumberAnimalsModal.innerHTML = `Se han elegido ( ${cant} ) animalitos`
...
function searchticketsbox(){
  let tickeIDBox = $("#TicketBox").val();
  var ul = '<ul class="collapsible" data-collapsible="accordion" id="ulBody">';
  var li = '';
  console.log(tickeIDBox);
  
  dbfirestore.collection('competitor').doc(UserUID).collection('bets').doc(tickeIDBox).get()
  .then(doc => { 
     var key = doc.id;
            ...
            var datestring = doc.data().playing;
            ...
            ul+=li;
    getID('divTicketsBox').innerHTML = ul;
